...
export async function fetchNode(hash: string) {
    const response = await fetch(`/kimap_explorer:kimap_explorer:doria.kino/api/node/${hash}`);
    return await response.json();
}

export async function fetchNodeInfo(hash: string) {
    const response = await fetch(`/kimap_explorer:kimap_explorer:doria.kino/api/info/${hash}`);
    return await response.json();
}
