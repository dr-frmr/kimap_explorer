...
export async function fetchNode(hash: string) {
    const response = await fetch(`/kimap_explorer:kimap_explorer:hexencode1.os/api/node/${hash}`);
    return await response.json();
}

export async function fetchNodeInfo(hash: string) {
    const response = await fetch(`/kimap_explorer:kimap_explorer:hexencode1.os/api/info/${hash}`);
    return await response.json();
}
